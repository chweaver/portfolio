...
| PDF tooling | pdfkit (dev only, for scripts/build-resume.mjs) | `^0.19.1` |
...
|   |- build-resume.mjs           renders public/Charlie-Weaver-Resume.pdf from its RESUME content block via pdfkit
...
|   |- Charlie-Weaver-Resume.pdf  GENERATED by scripts/build-resume.mjs (do not hand-edit); hero Resume CTA, served via publicAsset()
...
node scripts/build-resume.mjs        # regenerate public/Charlie-Weaver-Resume.pdf after editing its RESUME block
...
**Update the resume:** edit the `RESUME` content block at the top of
`scripts/build-resume.mjs`, run `node scripts/build-resume.mjs`, open
`public/Charlie-Weaver-Resume.pdf` to check the layout (it must stay one page),
then commit the script and the PDF together. The PDF is generated; never edit it
directly, and keep its claims in sync with the site (section 10 rules apply).
...
- **Do not edit generated or ignored files:** `out/`, `.next/`, `next-env.d.ts`,
  `public/og-card.png`, `public/Charlie-Weaver-Resume.pdf` (regenerate both via
  their scripts), or anything under `.claude/`.
...
- **sharp** is the only native dependency (used by `build-og-card.mjs` and
  `convert-screenshots.mjs`); it installs prebuilt binaries via npm. **pdfkit**
  (pure JS) powers `build-resume.mjs`.
